...
class GoogleManager:
    def __init__(self):
        ...
        # Scopes for Contacts and Sheets
        self.SCOPES = [
    ...
    async def load_credentials(self):
        ...
        if self.creds:
            try:
                ...
                # print("✅ Google Services (People + Sheets) Connected")
            except Exception as e:
    ...
    # --- Sheets API ---
    async def create_spreadsheet(self, title):
    ...
    async def add_sheet(self, spreadsheet_id, title):
        """Adds a new tab/worksheet. Returns sheetId or None if exists."""
        if not self.sheets_service: return None
        ...
        try:
            ...
            print(f"❌ Add Tab Error: {e}")
            return None

    async def write_values(self, spreadsheet_id, range_name, values):
        """Writes data to a range (e.g., 'Sheet1!A1')"""
        ...
        try:
            ...
            print(f"❌ Write Values Error: {e}")
            return False

    async def format_header(self, spreadsheet_id, sheet_id, start_row_index, end_row_index, start_col_index, end_col_index):
        """Bold header row"""
